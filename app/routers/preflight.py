...
def preflight_gpt(ir: PanelScheduleIR):
    ...
    if isinstance(main_cb, float) and isinstance(main_bus, float) and main_cb > main_bus:
        ...
        items.append(QAItem(check="MCB <= BUS AMPACITY", pass_=False, notes=f"MCB {main_cb}A > Bus {main_bus}A"))
    else:
        items.append(QAItem(check="MCB <= BUS AMPACITY (or MLO)", pass_=True))
