...
lyr_lighting = cfg.layers.get('lighting', 'E-LITE-FIXT'); _ensure_layer(doc, lyr_lighting)
    ...
    msp.add_text(f"{req.project} - Lighting Plan", dxfattribs={"height":0.3, "layer":lyr_ann}).set_pos((0,7), align=TextEntityAlignment.LEFT)
    ...
    for d in req.devices:
        ...
        if tag.startswith("L"):
            x,y = d.x, d.y
            msp.add_lwpolyline([(x-0.08,y-0.08),(x+0.08,y-0.08),(x,y+0.08),(x-0.08,y-0.08)], dxfattribs={"closed": True, "layer":lyr_rooms})
            msp.add_text(d.tag, dxfattribs={"height":0.12, "layer":lyr_ann}).set_pos((x+0.1, y-0.05))
        elif tag.startswith("S"):
            msp.add_circle((d.x,d.y), radius=0.05, dxfattribs={"layer":lyr_lighting})
            msp.add_text("S", dxfattribs={"height":0.12, "layer":lyr_ann}).set_pos((d.x+0.1, d.y-0.05))
        else:
            msp.add_circle((d.x,d.y), radius=0.05, dxfattribs={"layer":lyr_lighting})
